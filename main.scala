...
def main(args: String*): Unit = {
  val executor = args.indexOf("--executor") match {
    case idx if idx >= 0 && idx + 1 < args.length =>
      args(idx + 1)
    case _ =>
      args.indexOf("--llm") match {
        case idx if idx >= 0 && idx + 1 < args.length =>
          args(idx + 1)
        case _ =>
          System.err.println("Error: Missing required argument. Please specify the executor name using '--executor <name>' or '--llm <name>'.")
          sys.exit(1)
      }
  }
  ...
  try {
    ...
    val issuesJson = os.proc("gh", "issue", "list", "--state", "open", "--limit", "1000", "--json", "number,title,body,state").call().out.text()
    
    val json = ujson.read(issuesJson)
    val issues = json.arr.map { item =>
      val number = item("number").num.toInt
      val title = item("title").str
      val body = if (item.obj.contains("body") && !item("body").isNull) item("body").str else ""
      val state = if (item.obj.contains("state") && !item("state").isNull) item("state").str else ""
      Issue(number, title, body, state)
    }.toList
    
    val quotedExecutor = java.util.regex.Pattern.quote(executor)
    val ExecutorRegex = s"""(?i)(?:preferr?ed\\s+executor|executor)\\s*(?:is\\s+)?(?:is:?\\s+)?:?\\s*$quotedExecutor""".r
    def hasTargetExecutor(issue: Issue): Boolean = {
      ExecutorRegex.findFirstIn(issue.body).isDefined
    }

    val depLineKeywords = List("depends on", "depend on", "dependency", "dependencies", "parent")
    ...
    println(s"Found ${candidateTasks.size} open tasks with '$executor' preferred executor.")

    val nextTaskOpt = candidateTasks.find(task => !hasUnresolvedDependencies(task))
    nextTaskOpt match {
      case None =>
        println(s"No tasks found without unresolved dependencies and with '$executor' executor.")
      case Some(nextTask) =>
        ...
        val parentIdOpt = ParentRegex.findFirstMatchIn(nextTask.body).map(_.group(1).toInt)
        val parentConclusionOpt = parentIdOpt.flatMap { parentId =>
          println(s"Found parent task #$parentId. Fetching conclusion comment...")
          try {
            val res = os.proc("gh", "issue", "view", parentId.toString, "--json", "comments").call()
            ...
            case e: Exception =>
              println(s"Failed to read comments for parent task #$parentId: ${e.getMessage}")
              None
          }
        }
        ...
        if (os.exists(worktreePath)) {
          println(s"Worktree directory $worktreePath already exists. Cleaning up...")
          try {
            os.proc("git", "worktree", "remove", "--force", worktreePath.toString).call(cwd = os.pwd)
          } catch {
        ...
        val branchExists = try {
          os.proc("git", "show-ref", "--verify", s"refs/heads/$branchName").call(cwd = os.pwd)
          true
        } catch {
        ...
        if (branchExists) {
          ...
          os.proc("git", "worktree", "add", worktreePath.toString, branchName).call(cwd = os.pwd)
        } else {
          println(s"Creating branch $branchName and adding worktree")
          os.proc("git", "worktree", "add", "-b", branchName, worktreePath.toString).call(cwd = os.pwd)
        }

        // Run task there
        val parentConclusionStr = parentConclusionOpt.map(c => s"\nParent Task Conclusion Comment:\n$c\n").getOrElse("")
...
"""
        ...
        val output = runCommandAndCapture(Seq(executor, "-p", prompt), worktreePath)
...
```"""
        ...
        os.proc("gh", "issue", "comment", taskId.toString, "--body-file", tempFile.toString).call(cwd = os.pwd)

        // Check if there was files changed
        val gitStatus = os.proc("git", "status", "--porcelain").call(cwd = worktreePath).out.text().trim
        ...
        if (filesChanged) {
          ...
          os.proc("git", "commit", "-m", s"Implement task #$taskId: ${nextTask.title}").call(cwd = worktreePath)

          val hasRemote = os.proc("git", "remote").call(cwd = os.pwd).out.text().trim.nonEmpty
          if (hasRemote) {
            println("Pushing branch to origin...")
            os.proc("git", "push", "-u", "origin", branchName).call(cwd = worktreePath)
            println("Creating Pull Request...")
            os.proc("gh", "pr", "create", "--title", s"Task #$taskId: ${nextTask.title}", "--body", s"Closes #$taskId", "--head", branchName).call(cwd = worktreePath)
            println("Merging Pull Request...")
            os.proc("gh", "pr", "merge", "--merge", "--delete-branch").call(cwd = worktreePath)
          } else {
            println("No remote detected. Merging branch locally...")
            val mainBranch = os.proc("git", "branch", "--show-current").call(cwd = os.pwd).out.text().trim
        ...
        val closeComment = s"Task #$taskId completed successfully. Worktree closed."
        os.proc("gh", "issue", "close", taskId.toString, "--comment", closeComment).call(cwd = os.pwd)
...
def setIssueStatus(taskId: Int, status: String): Unit = {
  ...
  val (toAdd, toRemove) = if (status == "in progress") {
    (List("status: in progress", "in progress"), List("status: completed", "completed"))
  } else {
    (List("status: completed", "completed"), List("status: in progress", "in progress"))
  }
  
  try {
    val addFlags = toAdd.flatMap(l => Seq("--add-label", l))
    val removeFlags = toRemove.flatMap(l => Seq("--remove-label", l))
    val fullCmd = Seq("gh", "issue", "edit", taskId.toString) ++ addFlags ++ removeFlags
    os.proc(fullCmd).call()
  } catch {
    case e: Exception =>
      println(s"Warning: Failed to update GitHub labels for task #$taskId: ${e.getMessage}")
  }
