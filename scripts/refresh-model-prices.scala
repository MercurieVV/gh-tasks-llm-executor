...
@main def refreshModelPrices(sourceFile: String): Unit =
  ...
  val validatedEntries = prices.obj.get("prices").collect { case ujson.Arr(entries) =>
    entries.forall {
      case ujson.Obj(fields) =>
        List("agent", "model", "source", "asOfDate").forall(key =>
          ...
          List("inputUsdPerMTok", "outputUsdPerMTok").forall(key =>
            fields.get(key).collect { case ujson.Num(value) => value > 0 }.contains(true)
          )
      case _ => false
    }
  }
  validatedEntries match
    ...
    case _ =>
      println("Price catalog must contain a prices array with complete positive raw rates.")
