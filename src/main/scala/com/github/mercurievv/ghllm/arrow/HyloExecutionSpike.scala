...
/** `RecursiveArrows.executeRecursive` (`BusinessArrows.scala`) re-expressed as a single `scheme.hyloM` over
  * `TaskGraph.Node` instead of hand-rolled `ArrowDefer.fix` + `ArrowTraverse.untilLeft`. Wired in at `Wiring.scala` via
  * [[wire]], replacing `logged.executeRecursive` at both call sites (`runRootOnce`, `runOnce`) - see
  ...
  *
  * Design:
  *   - `M[A] = EitherT[F, RunSummary, A]`. `Left` always means "the walk is done, this is the final summary" - both the
  *     ordinary success case and every short-circuit case use it, so the outermost caller just needs `.merge`.
  ...
  *     `Applicative` skips constructing (never mind running) any later sibling's effect once one child is `Left`. That
  *     is what reproduces `ArrowTraverse.untilLeft`'s "stop at the first incomplete dependency, don't touch the rest"
  *     without writing the stop condition by hand.
...
object HyloExecutionSpike:
  ...
  /** Same contract as `RecursiveArrows.executeRecursive`: dependencies (and, for an already-split task, open children)
    * run before the node itself, in order, stopping at the first one that does not close.
    */
  ...
  /** Wires this traversal in place of `RecursiveArrows.executeRecursive`'s `ArrowDefer.fix` + `ArrowTraverse.untilLeft`
    ...
    * exist, `Replayability.lowerOrRaise` has already folded replay-or-real into them and `withArrowLogging` has already
    * wrapped each one, so this fold inherits both for free. `recordDependencyOutcome`/`routeDependencyOutcome` are
    * intentionally unused here: their logic (stop on a non-"completed" summary; block on `hasOpenChildren`) is folded
    * into this module's algebra directly (see the class doc), so those two fields' own enter/exit trace lines no longer
    * fire for this traversal - the record keeps them for `RecursiveArrows.executeRecursive`'s own tests and for
    * `Functor2K`/`Replayability` derivation, which still walk every field of the record regardless of which method
    * actually calls them.
    */
  def wire[F[_]: Async](
